...
# Ehrr, a bit too many of these, I got better things to do..
...
%define snapshot		20090315
%define release			%{manbo_mkrel 0.2}
...
Patch101: gcc44-pass-slibdir.patch
# pass libdir around
Patch107: gcc44-multi-do-libdir.patch
...
# ada
Patch105: gcc-4.4.0-ada-link.patch
Patch106: gcc44-ada-makefile.patch
# gnu pascal
Patch111: gcc34-gpc-fixes.patch
Patch112: gcc33-gpc-serialize-build.patch

# build 32 bits code when running through linux32
Patch115: gcc44-linux32.patch
# ?
Patch116: gcc44-linux32-build-env.patch
...
%if !%{system_compiler} && !%{build_minimal}
# We need gcc4.2 + its libstdc++ headers
%define gcc_version %((cpp --version 2>/dev/null || echo 0 ) |cut -d\\  -f3|head -n1)
%if !%{build_stdcxxheaders}
%define libstdcxx_includedir %{target_prefix}/include/c++/%{gcc_version}
%endif
%endif
%if %{build_ada}
...
%endif
%if "%{_real_vendor}" == "manbo" && !%{build_cross}
...
%if !%{system_compiler}
%if %{libc_shared}
Requires:	%{libstdcxx_name} >= %{gcc_version}
%endif
%if !%{build_stdcxxheaders}
Requires:	%{libstdcxx_name_orig}-devel >= %{gcc_version}
%endif
%endif
%endif
...
# strict version requires on exactly autoconf 2.59, screw that! Just making an
# empty file to avoid errors while trying to include seems sufficient..?</dirty>
...
rm -f %{buildroot}%{target_libdir}/libstdc++.so.%{libstdcxx_major}.0.%{libstdcxx_minor}
rm -f %{buildroot}%{gcc_libdir}/%{gcc_target_platform}/%{version}/libstdc++.a
%if %isarch %{biarches}
rm -f %{buildroot}%{_prefix}/lib/libstdc++.so.%{libstdcxx_major}
rm -f %{buildroot}%{_prefix}/lib/libstdc++.so.%{libstdcxx_major}.0.%{libstdcxx_minor}
rm -f %{buildroot}%{gcc_libdir}/%{gcc_target_platform}/%{version}/32/libstdc++.a
%endif
%endif
...
%{_bindir}/%{gcc_target_platform}-gcj%{program_suffix}
...
%{_bindir}/%{gcc_target_platform}-gcjh%{program_suffix}
%{_bindir}/jcf-dump%{program_suffix}
%{_bindir}/rebuild-gcj-db%{program_suffix}
# FIXME(?): Why not install in regular location for python modules?
%dir %{_datadir}/python
